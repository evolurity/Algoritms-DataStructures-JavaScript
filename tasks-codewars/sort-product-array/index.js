...
const parseStringToNumber = string => parseInt(string.replace(/\s/g, ''))

const isObject = obj => (typeof obj === 'object' && obj !== null)

const getPriceFromObj = obj => {
    let value
    if (isObject(obj.price)) {
        value = parseStringToNumber(obj.price.newUan)
    } else {
        value = parseStringToNumber(obj.price)
    }
    return value
}

const getFeedbackFromObj = obj => parseInt(obj.ratingRevievs.replace(/\s/g, ''))

const sortByPrice = array => {
    return array.sort((a, b) => {
        let priceA = getPriceFromObj(a)
        let priceB = getPriceFromObj(b)
        return priceB - priceA
    })
}

const sortByFeedbacks = array => array.sort((a, b) => getFeedbackFromObj(b) - getFeedbackFromObj(a))

const sortByPriceAndFeedback = array => {
    return array.sort((a, b) => {
        let priceA = getPriceFromObj(a)
        let priceB = getPriceFromObj(b)
        if (priceA === priceB) {
            return getFeedbackFromObj(b) - getFeedbackFromObj(a)
        }
        return priceB - priceA
...
const getTemlate = (obj) => {
    return `<li>
        <p>Name: ${obj.name}</p>
        <p>Price: ${getPriceFromObj(obj)}</p>
        <p>Feedback: ${obj.ratingRevievs}</p>
    </li>
    <hr>`
}

const listContainer = document.getElementById('listProducts')

const render = (data) => {
    listContainer.innerHTML = data.map(obj => getTemlate(obj)).join('')
}

document.getElementById('sortByPriceAndFeedback').onclick = () => {
    sortByPriceAndFeedback(data)
    render(data)
}

document.getElementById('sortByPrice').onclick = () => {
    sortByPrice(data)
    render(data)
}

document.getElementById('sortByFeedback').onclick = () => {
